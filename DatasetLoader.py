...
if __name__ == "__main__":
    ...
    train_list = "G:/Paper/Dataset/voxeceleb/mfa_conformer-master/data/train_vox2.txt"
    train_path = "G:/Paper/Dataset/voxeceleb/mfa_conformer-master/data/wav"

    train_dataset = train_dataset_loader(train_list=train_list, augment=False, musan_path="", rir_path="", max_frames=200, train_path=train_path)
    train_sampler = train_dataset_sampler(data_source=train_dataset, num_utt=2, max_seg_per_spk=600, num_spk=100, distributed=False, seed=10)
    ...
    for data, lable in train_loader:
        print(data.shape)  # torch.Size([2, 2, 32000])
        print(lable)  # tensor([284, 230])
        idx += 1
        if idx == 2:
            break